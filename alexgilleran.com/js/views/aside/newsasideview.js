define([ 'jquery', 'underscore', 'backbone', 'handlebars',
		'js/models/news/news', 'js/views/aside/news/tweetview',
		'js/views/aside/news/githubeventview',
		'text!templates/asideframe.html', ], function($, _, Backbone,
		Handlebars, News, TweetView, GithubEventView, AsideFrameTemplate) {
	var NewsAsideView = Backbone.View.extend({
		...
		initialize : function() {
			...
			this.listenTo(this.model, 'add', this.addNewsItem);
		...
		addNewsItem : function(newsItem) {
			var newsItemView = new this.itemViews[newsItem.get('source')]({
				model : newsItem
			});
